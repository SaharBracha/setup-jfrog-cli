...
export class Utils {
    ...
    // Application key to associate entities
    private static readonly APPLICATION_KEY: string = 'application-key';
    ...
    private static readonly JFROG_CONFIG_DIR_NAME: string = '.jfrog';
    // Config file name
    private static readonly JFROG_CONFIG_FILE: string = 'config.yml';
    ...
    private static async getApplicationKey(): Promise<string> {
        const appKey: string = core.getInput(Utils.APPLICATION_KEY);
        if (appKey) {
            return appKey;
        }

        const configFilePath: string = path.join(this.JFROG_CONFIG_DIR_NAME, this.JFROG_CONFIG_FILE);
        try {
            ...
            const configObj: any = YAML.parse(config);
